...
void FShaderCompileUtilities::ApplyFetchEnvironment(FShaderMaterialPropertyDefines& SrcDefines, const FShaderCompilerEnvironment& Environment)
{
	...
	//FETCH_COMPILE_BOOL 用于判断编译环境中是否有对应的宏
	//若没有，将 FShaderMaterialPropertyDefines 设置为0 修改
	...
	ERHIFeatureLevel::Type FEATURE_LEVEL)
{
	...
	if(Mat.MATERIAL_SHADINGMODEL_TOON || Mat.MATERIAL_SHADINGMODEL_TOONFACE)
	{
		//用于为不同 shading model开启需要使用的 GBufferslot 修改
		SetStandardGBufferSlots(Slots,bWriteEmissive,bHasTangent,bHasVelocity
			,bHasStaticLighting,bIsStrataMaterial);
		Slots[GBS_CustomData]= bUseCustomData;
	}
}
