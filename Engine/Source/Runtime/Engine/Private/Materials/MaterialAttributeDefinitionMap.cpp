...
FText FMaterialAttributeDefinitionMap::GetAttributeOverrideForMaterial(const FGuid& AttributeID, UMaterial* Material)
{
	...
	case MP_CustomData0:
		...
		//将 Toon 与 ToonFace 的 CustomData0 设置为 ToonData0
		CustomPinNames.Add({ MSM_Toon,"ToonData0"});
		CustomPinNames.Add({ MSM_ToonFace,"ToonData0"});
		return FText::FromString(GetPinNameFromShadingModelField(Material->GetShadingModels(), CustomPinNames, "Custom Data 0"));
	case MP_CustomData1:
		//将 Toon 与 ToonFace 的 CustomData1 设置为 ToonData1
		CustomPinNames.Add({ MSM_Toon,"ToonData1"});
		CustomPinNames.Add({ MSM_ToonFace,"ToonData1"});
